...
@Component({
  ...
  <div class="bottom-navigation fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around items-center py-2 md:hidden">
    ...
    >
      ...
      <span class="text-xs">ChatList</span>
      <div class="absolute top-0 right-1 bg-red-500 text-white rounded-full w-4 h-4 text-xs flex items-center justify-center" *ngIf="chatStore.allUnreadCount() > 0">
          {{ chatStore.allUnreadCount() }}
        </div>
    </button>
    <button class="flex flex-col items-center text-gray-600" (click)="userService.logout()">
