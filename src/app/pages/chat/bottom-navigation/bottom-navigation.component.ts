...
@Component({
  ...
  <div class="bottom-navigation fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around items-center py-2 md:hidden">
    ...
    >
      ...
      <div 
        class="absolute top-0 right-[-5px] bg-red-500 text-white rounded-full w-4 h-4 text-xs flex items-center justify-center" 
        *ngIf="chatStore.allUnreadCount() > 0"
        [ngClass]="{'w-8 right-[-15px]': chatStore.allUnreadCount() > 99}"
      >
